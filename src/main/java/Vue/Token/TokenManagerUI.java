...
public class TokenManagerUI {
    ...
    public TokenManagerUI(Level level, GamePanel gamePanel) {
        this.level = level;
    ...
    public void drawPlacedTokens(Graphics2D g2d, int widthOffset, int heightOffset) {
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                ...
                if (token != null) {
                    drawToken(g2d, token, x, y, tileSize);
                }
            }
        }
    }
    ...
    private void drawToken(Graphics2D g2d, Token token, int x, int y, int tileSize) {
        ...
        } else if (token instanceof Target) {
            Target receiver = (Target) token;
